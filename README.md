...
- **🌐 多站点适配**：原生支持多个主流磁力网站，如 **动漫花园 (dmhy.org)**、xl720、6v520 等，并能自动处理不同站点的链接和日期格式。
- **🛠️ 抓取引擎优化**：
  - **扩展版**：支持 **直接抓取 (Direct Fetch)**，跳过 CORS 代理，稳定性极高，速度极快。
  - **网页版**：内置多重 CORS 代理切换引擎，确保跨域访问成功。
  - **智能解码**：自动识别 `GBK/GB2312` 编码（如 6v520 等站点），彻底解决中文乱码。
- **📋 一键批量操作**：支持单条复制、卡片全部复制、以及全局“复制所有最新”功能。
- **💎 极简美学 UI**：采用 **Premium Glassmorphism (毛玻璃)** 视觉风格。
  - **多主题支持**：内置 **白天/夜间/跟随系统** 三种主题模式，一键随心切换。
  - **响应式设计**：适配各种屏幕尺寸，支持在 **新标签页 (Tab)** 中全屏打开，提供沉浸式阅读体验。
- **🔒 隐私与持久化**：
  - **Native Storage**：扩展版使用 `chrome.storage.local` 进行数据持久化。
  - **Privacy First**：数据完全存储在本地，无需后端服务器，保障个人隐私安全。
...
#### 方案 A：Chrome 插件（推荐 🌟）
插件版拥有更高的权限，不再受跨域限制（CORS），抓取更精准且速度更快。
...
#### 方案 B：本地网页版
由于浏览器跨域策略限制，网页版建议通过本地服务器运行。您可以直接运行脚本：
```bash
bash launch.sh
```
或者使用 python 快速启动：
...
1. **添加链接**：输入资源站点（如 动漫花园、xl720, 6v520 等）的详情页或搜索列表页 URL。
2. **筛选清晰度**：在顶部下拉框选择您需要的视频规格。
3. **切换主题**：点击右上角主题图标，可以在白天、夜间和系统自动模式间切换。
4. **批量复制**：点击“复制所有最新”，App 会自动将符合条件的磁链复制到剪贴板，并将它们标记为已读（从预览中消失）。
5. **刷新状态**：随时点击刷新按钮获取网站的最新变动。
